...
<!-- create modal -->
@include('admin.master.jenis.create')
<!-- create modal -->
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700" style="z-index: 15;">
            ...
            <!-- Modal body -->
                ...
                <div class="p-6 space-y-6">
                    <input type="hidden" value="" name="id_jenis" required />
                    <div>
...
<div id="hapusModal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-2xl max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700" style="z-index: 15;">
            ...
            <!-- Modal body -->
            <form method="POST" enctype="multipart/form-data" action="{{ route('jenis.destroy') }}">
                @csrf
                <div class="p-6 space-y-6">
                    <input type="hidden" value="" name="id_jenis" id="id_jenis" required />
    ...
    $('body').on('click','#edit',function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                $.map(data,function(i) {
                    $('#id').val(i.id_jenis);
                    $('#nama_jenis').val(i.nama_jenis);
                    // $('#status option[value="' + i.status+ '"]').prop('selected', true);
                    $("#status_jenis").find(`option[value=${i.status}]`).prop('selected', true);
                })
            }
        })
    })
