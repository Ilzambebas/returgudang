...
<!-- edit modal -->
@foreach($jenis as $d)
<div id="editModal {{ $d->id_jenis }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-2xl max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700" style="z-index: 15;">
            <!-- Modal header -->
            ...
            <form method="POST" action="{{ route('jenis.store') }}"  enctype="multipart/form-data">
                @csrf
                <div class="p-6 space-y-6">
                    <input type="hidden" value="{{ $d->id_jenis }}" name="id_jenis" required />
                    <div>
                        <label class="block text-sm">
                            Nama Bidang
                        </label>
                        <input type="text" value="{{ $d->nama_jenis }}" class="placeholder:text-gray-500 text-sm focus:shadow-primary-outline leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-blue-500 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow" name="nama_bidang" placeholder="Nama Bidang" required />
                    </div>
                    <div>
                        <label class="block text-sm">
                            Status
...
<!-- hapus modal -->
@foreach($jenis as $g)
<div id="hapusModal{{ $g->id_jenis }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-2xl max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700" style="z-index: 15;">
            <!-- Modal header -->
            ...
            <!-- Modal body -->
            <form method="GET" enctype="multipart/form-data" action="/jenis/{{ $g->id_jenis}}/destroy">
                @csrf
                <div class="p-6 space-y-6">
                    <input type="hidden" value="{{ $g->id_jenis }}" name="id_jenis" required />
                ...
                <div class="flex items-center justify-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                    <div class="">
                    <a href="{{ route('jenis.jenis') }}"class="bg-danger text-white py-2 px-4 rounded shadow-lg flex items-start mx-3">Tidak</a>
                    </div>
                    <div class="">
                        <button href="{{ route('jenis.jenis') }}" type="delete" class="bg-warning text-white py-2 px-4 rounded shadow-lg flex items-start mx-3">Yakin</button>
                    </div>
                </div>
            </form>
        </div>
                                ...
                                <tbody class="border-t">
                                    ...
                                    <tr>
                                        ...
                                        <div class="flex items-center justify-center">
                                                <button data-modal-target="editModal{{ $d->id_jenis}}" data-modal-toggle="editModal {{ $d->id_jenis}}" class="bg-warning text-white py-2 px-4 rounded shadow-lg flex items-center mx-3">
                                                    <div class="font-sm">Edit</div>
                                                    <div class="content-center mx-1">
                                                        <img src="{{ asset('img/edit.svg') }}" alt="">
                                                    </div>
                                                </button>
                                                <button data-modal-target="hapusModal{{ $g->id_jenis}}" data-modal-toggle="hapusModal{{ $g->id_jenis}}" class="bg-danger text-white py-2 px-4 rounded shadow-lg flex items-center">
