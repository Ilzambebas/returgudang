...
<!-- edit modal -->
@foreach($bidang as $d)
<div id="editModal {{ $d->id_bidang }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-2xl max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700" style="z-index: 15;">
            <!-- Modal header -->
            <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                ...
                <button href="{{ route('bidang.bidang') }}" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="editModal">
                    <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                    </svg>
                    <span class="sr-only">Close modal</span>
            ...
            <!-- Modal body -->
            <form method="POST" action="{{ route('bidang.store') }}"  enctype="multipart/form-data">
                @csrf
                <div class="p-6 space-y-6">
                    <input type="hidden" value="{{ $d->id_bidang }}" name="id_bidang" required />
                    <div>
                        <label class="block text-sm">
                            Nama Bidang
                        </label>
                        <input type="text" value="{{ $d->nama_bidang }}" class="placeholder:text-gray-500 text-sm focus:shadow-primary-outline leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-blue-500 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow" name="nama_bidang" placeholder="Nama Bidang" required />
                    </div>
                    <div>
                        <label class="block text-sm">
                            Status
...
<!-- hapus modal -->
@foreach($bidang as $g)
<div id="hapusModal{{ $g->id_bidang }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative w-full max-w-2xl max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700" style="z-index: 15;">
            <!-- Modal header -->
            ...
            <!-- Modal body -->
            <form method="GET" enctype="multipart/form-data" action="/bidang/{{ $g->id_bidang}}/destroy">
                @csrf
                <div class="p-6 space-y-6">
                    <input type="hidden" value="{{ $g->id_bidang }}" name="id_bidang" required />
                ...
                <div class="flex items-center justify-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                    ...
                    <div class="">
                        <button href="{{ route('bidang.bidang') }}" type="destroy" class="bg-warning text-white py-2 px-4 rounded shadow-lg flex items-start mx-3">Yakin</button>
                    </div>
                </div>
            </form>
        </div>
                            ...
                            <table class="items-center justify-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                                <thead class="align-bottom">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">Nama Bidang</th>
                                ...
                                <tbody class="border-t">
                                    ...
                                    <tr>
                                        ...
                                        <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                            <p class="mb-0 text-sm font-semibold leading-normal dark:text-white dark:opacity-60">{{ $row->nama_bidang }}</p>
                                        </td>
                                        <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                        ...
                                        <div class="flex items-center justify-center">
                                                <button data-modal-target="editModal{{ $d->id_bidang}}" data-modal-toggle="editModal {{ $d->id_bidang}}" class="bg-warning text-white py-2 px-4 rounded shadow-lg flex items-center mx-3">
                                                    <div class="font-sm">Edit</div>
                                                    <div class="content-center mx-1">
                                                        <img src="{{ asset('img/edit.svg') }}" alt="">
                                                    </div>
                                                </button>
                                                <button data-modal-target="hapusModal{{ $g->id_bidang}}" data-modal-toggle="hapusModal{{ $g->id_bidang}}" class="bg-danger text-white py-2 px-4 rounded shadow-lg flex items-center">
